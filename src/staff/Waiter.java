...
public class Waiter extends Staff implements Billable {
  ...
  public void takeBillsFromTable() {
    for (Table table : tables) {
      this.moneyFromClients = this.moneyFromClients.add(table.getTableBill()
  ...
  public void takeTipFromTable() {
    for (Table table : tables) {
      this.tipFromClients = this.tipFromClients.add(table.getTableTip()
          .setScale(2, RoundingMode.HALF_UP));
    }
  }
  ...
  public void addsBonusToSalary() {
    ...
    if (bonus > 6) {
      System.out.println("Добавни 150лв. към заплатата на " + getName() + " !");
      setSalary(getSalary().add(BigDecimal.valueOf(150)));
